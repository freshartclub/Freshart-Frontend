    ...
    <div className={`px-4 py-6 ${dark ? "bg-gray-900" : "bg-gray-50"} min-h-screen`}>
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-4">
          <div>
            <h1 className={`text-2xl font-bold ${dark ? "text-white" : "text-gray-800"}`}>{t("Subscription Orders")}</h1>
            <span className={dark ? "text-gray-400 text-sm" : "text-gray-400 text-sm"}>{t("View/Manage your all subscription orders")}</span>
          </div>
