...
const CircleMetric = ({ title, value, darkMode = false, icon }: MetricProps) => {
  ...
  return (
    <div className={`flex flex-col items-center ${colors.cardBg} rounded-lg p-4 h-full ${colors.shadow} transition-all duration-300 hover:scale-[1.02]`}>
      ...
      <div className="w-full h-32 flex items-end justify-around mt-4">
        {barData.map((height, idx) => (
          <div key={idx} className="flex flex-col items-center group">
            ...
            <span className={`${colors.textSecondary} text-xs mt-1`}>
      ...
      {tabs.map(tab => (
        <button
          ...
          className={`py-2 px-4 text-sm rounded-md mx-1 transition-all duration-200 ${
            activeTab === tab 
              ? `${colors.primary} ${darkMode ? 'text-white' : 'text-white'} font-medium` 
...
const TabSelector = ({ activeTab, setActiveTab, darkMode = false }: { 
  activeTab: string; 
  setActiveTab: (tab: string) => void;
  darkMode?: boolean;
}) => {
  ...
  return (
    <div className={`w-full px-4 mx-auto pt-6 ${colors.bg} min-h-screen`}>
      ...
      <div className="flex flex-col md:flex-row gap-4 mb-6">
        <div className={`${colors?.primary} ${darkMode ? 'text-gray-100' : 'text-white'} text-xl font-bold p-4 rounded-lg w-full shadow-md flex items-center justify-center`}>
      ...
      <div className="flex justify-end py-4">
        <div className={`${colors.textSecondary} text-xs`}>
          Dashboard v1.2.4 • Theme: {darkMode ? 'Dark' : 'Light'}
        </div>
      </div>
    </div>
  );
