...
const Offers = () => {
  ...
  return (
    <div className={`p-4 min-h-full ${dark ? 'bg-gray-900 text-white' : ''}`}>
        ...
        <table className={`min-w-full divide-y ${dark ? 'divide-gray-700' : 'divide-gray-200'}`}>
          <thead className={dark ? 'bg-gray-800' : 'bg-gray-50'}>
            <tr>
              ...
              <th className={`px-6 py-3 text-left text-xs font-medium uppercase tracking-wider ${dark ? 'text-gray-300' : 'text-gray-500'}`}>Initial Offer</th>
              ...
              <th className={`px-6 py-3 text-left text-xs font-medium uppercase tracking-wider ${dark ? 'text-gray-300' : 'text-gray-500'}`}>Actions</th>
              <th className={`px-6 py-3 text-left text-xs font-medium uppercase tracking-wider ${dark ? 'text-gray-300' : 'text-gray-500'}`}>Details</th>
            </tr>
          </thead>
          <tbody className={dark ? 'bg-gray-900 divide-gray-700' : 'bg-white divide-gray-200'}>
            {paginatedData.map((offer: Offer) => {
              const latestCounterOffer = offer?.counterOffer && offer?.counterOffer.length > 0 
                ? offer?.counterOffer[offer?.counterOffer?.length - 1] 
                : null;
              const isExpanded = expandedOffers.includes(offer._id);

              console.log(latestCounterOffer)
              ...
              return (
                <React.Fragment key={offer._id}>
                  <tr className={isExpanded ? `${dark ? 'bg-gray-800' : 'bg-gray-50'}` : ''}>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        ...
                        <div className={`text-sm ${dark ? 'text-gray-100' : 'text-gray-900'}`}>
                          {offer.artwork.artworkName}
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className={`text-sm ${dark ? 'text-gray-100' : 'text-gray-900'}`}>
                        {offer.user.artistName || offer.user.nickName}
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className={`text-sm ${dark ? 'text-gray-100' : 'text-gray-900'}`}>
                        €{latestCounterOffer?.offerprice}
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className={`text-sm ${dark ? 'text-gray-100' : 'text-gray-900'}`}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      {(!latestCounterOffer || latestCounterOffer.isAccepted === null) && (
                        <div className="flex gap-2">
                          <button
                            className={`px-3 py-1 text-sm font-medium text-white bg-green-500 rounded-md hover:bg-green-600 transition-colors ${actionInProgress === offer._id ? 'opacity-50 cursor-not-allowed' : ''}`}
                          ...
                          <button
                            className={`px-3 py-1 text-sm font-medium text-white bg-red-500 rounded-md hover:bg-red-600 transition-colors ${actionInProgress === offer._id ? 'opacity-50 cursor-not-allowed' : ''}`}
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <button
                        className={`px-3 py-1 text-sm font-medium ${dark ? 'bg-gray-700 text-gray-100' : 'bg-gray-200 text-gray-700'} rounded-md hover:bg-[#102030] hover:text-white transition-colors`}
                          ...
                          <div className={`px-4 py-5 ${dark ? 'bg-gray-800' : 'bg-white'} sm:p-6`}>
                            ...
                            <div className="mt-4 border-t border-b py-4">
                              ...
                              {offer.counterOffer && offer.counterOffer.length > 0 ? (
                                  ...
                                  {offer.counterOffer.map((counter, index) => (
                                    <div key={counter._id} className={`p-3 rounded ${dark ? 'bg-gray-700' : 'bg-gray-100'}`}>
                                      <div className="flex justify-between items-center">
                                        ...
                                        <div>
                                          {getStatusBadge(counter.isAccepted)}
                                        </div>
                                      </div>
                                      {counter.comment && (
                                        <div className={`mt-2 text-sm ${dark ? 'text-gray-300' : 'text-gray-500'}`}>
                                ...
                                <div className={`text-sm italic ${dark ? 'text-gray-400' : 'text-gray-500'}`}>
      ...
      {paginatedData.length === 0 && (
        <div className={`text-center py-12 ${dark ? 'text-gray-300' : 'text-gray-500'}`}>
      ...
      <div className="flex justify-between items-center mt-4">
        ...
        <div className="flex gap-2">
          <button
            className={`px-3 py-1 rounded hover:bg-gray-300 ${currentPage === 1 ? 'opacity-50 cursor-not-allowed' : ''} ${dark ? 'bg-gray-700 text-gray-100' : 'bg-gray-200'}`}
          ...
          <button
            className={`px-3 py-1 rounded hover:bg-gray-300 ${currentPage * itemsPerPage >= filteredData.length ? 'opacity-50 cursor-not-allowed' : ''} ${dark ? 'bg-gray-700 text-gray-100' : 'bg-gray-200'}`}
