...
const BannerSection = ({ data }) => {
  ...
  const getContent = data.find((item) => item?.type === "Main-Banner");

  const stripHtmlTags = (text) => {
    const paragraphs = text.split(/<\/?p>/g).filter((part) => part.trim() !== "");
    return paragraphs;
  };
    ...
    >
      <div className="container mx-auto sm:px-6 px-3">
        <div className="xl:w-[60%] lg:w-[70%] mx-auto pt-[0px] text-white text-center">
          <h1 className="md:text-[80px] text-[50px] capitalize font-semibold">{getContent?.title}</h1>
          <h1 className="md:text-[80px] text-[50px] capitalize font-semibold">{getContent?.subtitle}</h1>
          ...
          <div className="bg-white flex justify-between sm:mx-24 px-2 py-1 rounded-full">
            <input type="text" placeholder="email,phone or invite code" className="ml-2 text-black w-full" />
            <Button onClick={handleProceed} className="px-6 py-2 rounded-full bg-[#FF536B] text-white">
              Process
            </Button>
          </div>
