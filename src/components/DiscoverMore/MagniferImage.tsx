...
export const MagnifierImage = ({ src, alt, isOffensive, safeMode, enableZoom = false }) => {
  ...
  const handleZoomedMouseMove = useCallback((e) => {
  ...
  const handleZoomWheel = useCallback((e) => {
    ...
    setZoomLevelIndex(prevIndex => 
      Math.max(0, Math.min(zoomLevels.length - 1, prevIndex + direction))
    );
  ...
  return (
    <div
      ref={containerRef}
      className={`relative ${dark ? "bg-red-800" : "bg-zinc-100"} p-2 w-full h-full py-2`}
