...
const CommonCardData = ({ heading, highlightData, isLoading = false }: { heading: string; highlightData: HighlightItem[]; isLoading?: boolean }) => {
          ...
          {highlightData?.map((item, index: number) => {
            ...
            return (
              <div
                ...
                } hover:shadow-[5px_5px_5px_rgba(0,0,0,0.05)] transition-shadow duration-300 min-w-[230px] max-w-[300px] h-[313px] group`}
              >
                ...
                <div className="mt-3">
                  <div className="flex flex-col">
                    <h1 className={`font-bold text-[16px] ${dark ? "text-gray-100" : "text-[#333333]"}`}>
                    ...
                    <p className={`text-xs ${dark ? "text-gray-400" : "text-gray-500"}`}>
                      {`${item?.additionalInfo?.length || "--"} x ${item?.additionalInfo?.width || "--"} x ${item?.additionalInfo?.height || "--"}`}{" "}
                      cm
                    </p>
                  </div>

                  {item?.pricing?.basePrice ? (
                    <p className={`text-[14px] font-bold ${dark ? "text-gray-100" : "text-gray-800"}`}>
                      {getSymbolFromCurrency(item?.pricing?.currency.split(" ")[0]) + " " + item?.pricing?.basePrice}
                    </p>
                  ) : null}
                </div>
              </div>
  ...
  return (
    <div className="my-7">
      <h2 className={`sm:text-[23px] text-[20px] font-semibold mb-4 ${dark ? "text-gray-100" : "text-gray-800"}`}>{heading}</h2>
