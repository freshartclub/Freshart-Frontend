...
const ImagePositioningPopup = ({ onClose, artwork }) => {
  ...
  const themeClasses = useMemo(() => ({
    ...
    buttonPrimary: dark ? 'bg-blue-700 hover:bg-blue-600' : 'bg-blue-600 hover:bg-blue-700',
          ...
          <div className={`grid grid-cols-2 sm:grid-cols-3 gap-2 h-full max-h-64 overflow-y-auto p-2 ${themeClasses.secondaryBg} rounded-lg`}>
            {uploadedImages?.data?.length > 0 ? (
              uploadedImages.data.map((img) => (
                <div 
                  key={img._id}
                  className={`relative cursor-pointer rounded-md overflow-hidden border-2 ${selectedImage === img.image ? themeClasses.selectedBorder : 'border-transparent'}`}
  ...
  const Controls = useCallback(({ isFullscreen = false }) => {
    ...
    return (
      <div className={`${isFullscreen ? 'absolute bottom-16 left-1/2 transform -translate-x-1/2 bg-black bg-opacity-70 rounded-lg p-2 flex items-center z-10' : `absolute bottom-4 left-1/2 transform -translate-x-1/2 ${themeClasses.controlsBg} rounded-lg p-2 flex items-center`}`}>
        <button 
          onClick={handleZoomOutClick}
          className={`p-2 ${isFullscreen ? 'text-white hover:bg-gray-800' : `${themeClasses.controlsText} ${themeClasses.controlsHover}`} rounded-md`}
          aria-label="Zoom out"
        >
          <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
        ...
        <button
          ...
          className={`ml-2 p-2 ${isFullscreen ? 'text-white hover:bg-gray-800' : `${themeClasses.controlsText} ${themeClasses.controlsHover}`} rounded-md`}
  ...
  return (
    <div className={`fixed inset-0 ${themeClasses.modalBg} bg-opacity-50 flex items-center justify-center z-50 p-2 sm:p-4`} role="dialog" aria-modal="true" aria-labelledby="image-positioning-title">
      <div className={`${themeClasses.contentBg} rounded-xl shadow-2xl max-w-6xl w-full h-[90vh] max-h-[90vh] overflow-hidden flex flex-col`}>
        ...
        <div className={`flex justify-between items-center border-b ${themeClasses.border} p-3 sm:p-4`}>
          <h3 id="image-positioning-title" className="text-lg sm:text-xl font-semibold">Position Your Artwork</h3>
          <button 
            onClick={onClose}
        ...
        <div className="flex-1 flex flex-col md:flex-row p-2 sm:p-4 gap-3 sm:gap-4 overflow-hidden">
          ...
          >
            ...
            <div className={`absolute top-4 left-4 ${dark ? 'bg-gray-800 text-gray-200' : 'bg-white text-gray-700'} bg-opacity-70 rounded px-2 py-1 text-xs sm:text-sm`}>
