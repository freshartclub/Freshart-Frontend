...
// /**
//  * DiscoverContent Component - Displays detailed information about an artwork
//  * @param {Object} props - Component props
//  * @param {Object} props.data - Artwork data to display
//  */
const DiscoverContent = ({ data }) => {
  ...
  const checkCartItem = useMemo(() =>
    cartItem?.data?.cart?.filter(item => item?._id === data?._id) || [],
    [cartItem, data?._id]);

  const isInCart = useMemo(() => checkCartItem.length > 0, [checkCartItem]);

  const checkWishlist = useMemo(() =>
    likedItems?.likedArtworks?.filter(item => item?._id === data?._id) || [],
    [likedItems, data?._id]);
    ...
    <div className={textThemeClass}>
      ...
      <div className="mt-3 space-y-1">
        <div className="flex gap-2">
          ...
          <P
            className={`${secondaryTextClass} !text-[13px]`}
            variant={{ size: 'small', theme: dark ? 'light' : 'dark', weight: 'medium' }}
          >
      ...
      <div className={containerClasses}>
        {purchaseType === 'Fixed Price' || isSubscription ? <Button
        ...
        >
          <img src={cart} alt="" className="w-5 h-5 md:mx-2 mx-1" aria-hidden="true" />
        ...
        {isSubscription ? (
          <Button className="text-base flex items-center justify-center border rounded-full w-full">
            <P variant={{ size: 'base', theme: 'dark', weight: 'normal' }}>
              Purchase Type: {data?.commercialization?.purchaseOption}
            </P>
          </Button>
        ) : purchaseType === 'Fixed Price' ? null : (
      ...
      <div className="flex flex-wrap w-full items-center justify-between relative mt-5 px-1">
          ...
          {isFavorite === data?._id && (
            <div
              ...
              className={`absolute bottom-0 left-0 ${dark ? "bg-gray-800 border-gray-700" : "bg-white border-gray-200"
        ...
        <button aria-label="Like artwork" onClick={handleLikeToggle} className="transition-transform hover:scale-110">
          {isInWishlist ? (
            ...
            <AiOutlineLike size="1.5rem" color={iconColor} />
          )}
        </button>
