...
const UserAllCircle = ({ data }) => {
  ...
  const circleManager = useAppSelector(
    (state) =>
  ...
  const handleCircle = (id, type) => {
    if (profile === "user") {
      ...
      if (type === "Private") {
        ...
        navigate(
          `/artist-panel/circle/circlepage?id=${encodeURIComponent(id)}`
        );
      }
    }
  };
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-4">
        {data &&
          data?.data?.map((circle) => (
            <div className="flex gap-2 sm:flex-row sm:p-2 sm:mb-0 shadow-md rounded-lg border items-center">
              <div
